        <div class="space-y-4" x-data="{ openModal: false }" x-on:open-seat-modal.window="openModal = true"
                        ...
                        <div class="flex justify-between items-start">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2">
                                    ...
                                    @endphp
                                    <span
                                        class="text-[9px] font-bold px-1.5 py-0.5 rounded {{ $typeColors[$passenger['passenger_type'] ?? 'dewasa'] ?? 'bg-gray-100 text-gray-600' }}">
            ...
            @if ($this->showPassengerModal)
                <template x-teleport="body">
                    <div class="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center overflow-hidden"
                        role="dialog" aria-modal="true" x-data="{ show: true }" x-show="show"
                        x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                        x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200"
                        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">

                        {{-- Backdrop --}}
                        <div wire:click="closePassengerModal" class="fixed inset-0 bg-gray-900/60 backdrop-blur-md">
                        </div>
                        ...
                        <div class="relative bg-white w-full max-w-lg rounded-t-[2.5rem] sm:rounded-3xl shadow-2xl flex flex-col h-[90vh] sm:h-auto sm:max-h-[90vh] overflow-hidden z-[10000]"
                            ...
                            <div
                                ...
                                <button wire:click="closePassengerModal"
                            ...
                            <div class="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-4">
                                ...
                                {{-- Gender --}}
                                <div>
                                    <label class="text-xs font-semibold text-gray-500 uppercase ml-1">Jenis Kelamin
                                        <span class="text-red-500">*</span></label>
                                    <div class="grid grid-cols-2 gap-2 mt-1">
                                        <button type="button" wire:click="$set('form_gender', 'male')"
                                            class="py-3 text-sm font-bold rounded-xl border-2 transition-all flex items-center justify-center gap-2 {{ $this->form_gender === 'male' ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-100 text-gray-400' }}">
                                            <x-heroicon-o-user class="w-4 h-4" /> Laki-laki
                                        </button>
                                        <button type="button" wire:click="$set('form_gender', 'female')"
                                            class="py-3 text-sm font-bold rounded-xl border-2 transition-all flex items-center justify-center gap-2 {{ $this->form_gender === 'female' ? 'border-pink-600 bg-pink-50 text-pink-600' : 'border-gray-100 text-gray-400' }}">
                                ...
                                {{-- Kategori Usia --}}
                                <div>
                                    <label class="text-xs font-semibold text-gray-500 uppercase ml-1">Kategori Usia
                                        <span class="text-red-500">*</span></label>
                                        ...
                                        @foreach (['dewasa' => 'Dewasa', 'anak-anak' => 'Anak-anak', 'balita' => 'Balita'] as $val => $label)
                                            @php
                                                $activeColors = [
                                                    ...
                                                    'anak-anak' => 'border-yellow-600 bg-yellow-50 text-yellow-600',
                                                    'balita' => 'border-pink-600 bg-pink-50 text-pink-600',
                                                ];
                                            @endphp
                                            <button type="button"
                                                wire:click="$set('form_passenger_type', '{{ $val }}')"
                                                class="py-3 text-xs font-bold rounded-xl border-2 transition-all {{ $this->form_passenger_type === $val ? $activeColors[$val] : 'border-gray-100 text-gray-400' }}">
                                                {{ $label }}
                                            </button>
                                        @endforeach
                                    </div>
                                    @error('form_passenger_type')
                                        <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] text-gray-400 mt-1 ml-1">Isi 0 jika balita/gratis.</p>
                                ...
                                <div class="p-3 bg-emerald-50/50 rounded-xl border border-emerald-100 space-y-2">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <button type="button" wire:click="$toggle('form_need_pickup')"
                                            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors {{ $this->form_need_pickup ? 'bg-emerald-600' : 'bg-gray-300' }}">
                                            <span
                                                class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {{ $this->form_need_pickup ? 'translate-x-6' : 'translate-x-1' }}"></span>
                                        </button>
                                    </div>
                                ...
                                <div class="p-3 bg-purple-50/50 rounded-xl border border-purple-100 space-y-2">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <button type="button" wire:click="$toggle('form_need_dropoff')"
                                            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors {{ $this->form_need_dropoff ? 'bg-purple-600' : 'bg-gray-300' }}">
                                            <span
                                                class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {{ $this->form_need_dropoff ? 'translate-x-6' : 'translate-x-1' }}"></span>
                                        </button>
                                    </div>
                            ...
                            <div class="p-4 bg-white border-t shrink-0">
                                <button wire:click="savePassenger"
                                    class="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-lg active:scale-[0.98] transition-transform flex items-center justify-center gap-2">
