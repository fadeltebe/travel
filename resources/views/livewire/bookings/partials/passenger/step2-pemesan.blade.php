        <div class="space-y-4">
            <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" wire:model.live.debounce.300ms="booker_name"
                ...
                <div>
                    ...
                    <input type="tel" wire:model.live.debounce.300ms="booker_phone"
                ...
                <div class="flex items-center justify-between p-3 bg-blue-50/50 rounded-xl border border-blue-100">
                    ...
                    <button type="button" wire:click="$toggle('booker_is_passenger')"
                        class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors {{ $this->booker_is_passenger ? 'bg-blue-600' : 'bg-gray-300' }}">
                        <span
                            class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {{ $this->booker_is_passenger ? 'translate-x-6' : 'translate-x-1' }}"></span>
                    </button>
                </div>

                @if ($this->can_manage_all_agents)
                    <div class="relative">
                        <label class="text-xs font-semibold text-gray-500 uppercase ml-1">Pilih Cabang Agen *</label>
                        <select wire:model="agent_id"
                            class="w-full mt-1 px-4 py-3 rounded-xl border-gray-200 bg-white focus:ring-blue-500 focus:border-blue-500">
