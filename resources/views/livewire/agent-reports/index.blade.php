...
mount(function () {
    ...
    // Pastikan user memiliki agent_id (merupakan agen)
    if (!$user->agent_id) {
        abort(403, 'Hanya agen yang dapat mengakses halaman ini.');
    }
});
...
$metrics = computed(function () {
    ...
    $bookingsQuery = Booking::query()
        ...
        ->whereHas('schedule', function (Builder $q) use ($month, $year) {
            $q->whereMonth('departure_date', $month)->whereYear('departure_date', $year);
        });

    $passengersQuery = Passenger::query()
        ->whereHas('booking', function(Builder $q) use ($month, $year, $agentId) {
            $q->where('agent_id', $agentId)
              ->whereHas('schedule', function (Builder $sq) use ($month, $year) {
                  $sq->whereMonth('departure_date', $month)->whereYear('departure_date', $year);
              });
        });
...
<div>
    <x-layouts.app title="Laporan Agen">
        <div class="px-4 pb-24 space-y-6">

            {{-- Header, Filter & Revenue Combined --}}
            <div
                class="bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-2xl p-6 shadow-md text-white flex flex-col relative overflow-hidden mb-6">
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
                        <div>
                            <h1 class="text-xl font-bold font-sans">Laporan Agen</h1>
                            <p class="text-xs text-indigo-100 mt-0.5">Rekapitulasi operasional agen berbasis pencarian bulan
                            </p>
                        </div>
                    ...
                    <div>
                        <h2 class="text-[10px] font-bold text-indigo-100 uppercase tracking-widest mb-1">Total
                            Pendapatan Agen (Lunas)</h2>
                        <p class="text-4xl font-black mb-4">Rp
                            {{ number_format($this->metrics['revenue']['total'], 0, ',', '.') }}</p>

                        <div class="flex flex-wrap gap-4 pt-4 border-t border-indigo-400/30">
                            <div class="flex-1 sm:flex-none min-w-[120px]">
                                <p class="text-[10px] font-bold text-indigo-100 uppercase">Dari Penumpang</p>
                                <p class="text-lg font-bold">Rp
                                    {{ number_format($this->metrics['revenue']['passenger'], 0, ',', '.') }}</p>
                            </div>
                            <div class="w-px bg-indigo-400/30 hidden sm:block"></div>
                            <div class="flex-1 sm:flex-none min-w-[120px]">
                                <p class="text-[10px] font-bold text-indigo-100 uppercase">Dari Kargo Barang</p>
                                <p class="text-lg font-bold">Rp
            ...
            <div
                ...
                <div
                    ...
                    <span class="text-indigo-600">Berjalan: <span
                            class="text-indigo-800 font-bold">{{ $this->metrics['schedules']['ongoing'] }}</span></span>
                    <span class="text-gray-300">|</span>
            ...
            <div
                ...
                <div
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <span class="text-gray-300">|</span>
                        <span class="text-indigo-600">Pending: <span
                                class="font-bold text-indigo-800">{{ $this->metrics['cargos']['pending'] }}</span></span>
                    </div>
                    <div class="flex flex-wrap items-center gap-2">
