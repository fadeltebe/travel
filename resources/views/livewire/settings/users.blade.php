...
<div>
    <x-layouts.app title="Pengguna & Akses">
        <div class="max-w-3xl mx-auto pb-28">
                ...
                @forelse ($this->users as $u)
                    <div
                        ...
                        <div class="p-4 flex items-start gap-4">
                            {{-- Icon --}}
                            <div
                                class="w-12 h-12 rounded-xl bg-pink-50 text-pink-600 flex items-center justify-center shrink-0 font-bold text-lg uppercase">
                                {{ substr($u->name, 0, 1) }}
                            </div>
                            {{-- Info --}}
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex flex-wrap items-center gap-1.5 mt-2">
                                    <span class="text-[10px] font-bold bg-pink-50 text-pink-600 px-2 py-0.5 rounded-lg uppercase">
                                        {{ $u->role->label() ?? $u->role }}
                                    </span>
                                    @if ($u->agent)
                                        <span class="text-[10px] font-bold bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-lg truncate max-w-[120px]">
                                            {{ $u->agent->name }}
                                        </span>
                                    @endif
                                </div>
            ...
            {{-- FAB Tambah User --}}
            <button wire:click="openCreate" class="fixed right-4 z-40 w-14 h-14 rounded-full text-white flex items-center justify-center shadow-lg active:scale-95 transition-transform border-2 border-white/30" style="bottom: calc(72px + env(safe-area-inset-bottom)); background: linear-gradient(135deg, #db2777, #be185d); box-shadow: 0 4px 20px rgba(219,39,119,0.45);" title="Tambah Pengguna">
