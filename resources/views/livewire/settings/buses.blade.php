...
<div>
    <x-layouts.app title="Manajemen Armada">
        <div class="max-w-3xl mx-auto pb-28">
            ...
            <div class="relative overflow-hidden text-white mx-4 mt-2 mb-6 rounded-2xl px-4 pt-5 pb-10"
                ...
                <div class="mt-5 relative z-10">
                    <div class="relative">
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari nama bus atau plat..."
            ...
            <div class="px-4 -mt-4 space-y-4">
                @forelse ($this->buses as $bus)
                    <div
                        ...
                        {{-- Layout Info --}}
                        @if ($bus->busLayout)
                            <div class="px-4 bg-gray-50/80 border-t border-gray-100 flex items-center gap-2 justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs font-black text-purple-600">{{ $bus->total_seats }} kursi</span>
                            </div>
        ...
        {{-- FAB Tambah Bus --}}
        <button wire:click="openCreate" class="fixed right-4 z-40 w-14 h-14 rounded-full text-white flex items-center justify-center shadow-lg active:scale-95 transition-transform border-2 border-white/30" style="bottom: calc(72px + env(safe-area-inset-bottom)); background: linear-gradient(135deg, #4338ca, #4f46e5); box-shadow: 0 4px 20px rgba(79,70,229,0.45);" title="Tambah bus">
        ...
        @if ($showDeleteModal)
            <div class="fixed inset-0 z-[9999] flex items-center justify-center p-4" x-data x-cloak>
                ...
                <div
                    class="relative bg-white rounded-3xl shadow-2xl p-6 max-w-sm w-full z-10 text-center">
                    <div
