...
on(['openEditRoute' => function ($routeId) {
    $this->resetValidation();
...
$save = function () {
    ...
    $existingRouteQuery = Route::where('origin_agent_id', $this->origin_agent_id)
                                ->where('destination_agent_id', $this->destination_agent_id);
            ...
            <div x-show="show"
                ...
                <div class="p-6 overflow-y-auto custom-scrollbar flex-1 relative z-10 bg-white">
                    <form wire:submit="save" id="routeForm" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select wire:model="origin_agent_id" class="w-full px-4 py-3 rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-colors text-sm">
                                <option value="">-- Pilih Agen Asal --</option>
                                @foreach($this->agents as $agent)
                                    <option value="{{ $agent->id }}">{{ $agent->city }} - {{ $agent->name }}</option>
                                @endforeach
                            </select>
                            @error('origin_agent_id') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>

                        {{-- Destination --}}
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1.5">Agen Tujuan</label>
                            <select wire:model="destination_agent_id" class="w-full px-4 py-3 rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-colors text-sm">
                                <option value="">-- Pilih Agen Tujuan --</option>
                                @foreach($this->agents as $agent)
                                    <option value="{{ $agent->id }}">{{ $agent->city }} - {{ $agent->name }}</option>
                                @endforeach
                            </select>
                            @error('destination_agent_id') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="number" wire:model="distance_km" class="w-full px-4 py-3 rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-colors text-sm" placeholder="Misal: 150">
                                @error('distance_km') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-700 mb-1.5">Estimasi Waktu (Menit)</label>
                                <input type="number" wire:model="estimated_duration_minutes" class="w-full px-4 py-3 rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-colors text-sm" placeholder="Misal: 120">
                                @error('estimated_duration_minutes') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" wire:model="base_price" class="w-full px-4 py-3 rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-colors text-sm font-black text-teal-700" placeholder="0">
                ...
                <div class="p-4 sm:p-6 border-t border-gray-100 bg-gray-50 relative z-10 shrink-0">
                    <div class="flex gap-3">
                        ...
                        <button type="submit" form="routeForm" class="flex-1 py-3 px-4 rounded-xl text-white font-bold text-sm bg-teal-600 hover:bg-teal-700 active:scale-95 shadow-lg shadow-teal-200 transition-all flex items-center justify-center gap-2">
