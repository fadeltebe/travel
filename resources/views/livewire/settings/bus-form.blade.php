...
$selectedLayoutPreview = computed(function () {
    if (!$this->bus_layout_id) return null;
    return BusLayout::with(['seats' => fn($q) => $q->orderBy('row')->orderBy('column')])
        ->find($this->bus_layout_id);
});

on(['openCreateBus' => function () {
    $this->resetFormState();
    $this->showFormModal = true;
}]);

on(['openEditBus' => function ($busId) {
...
$saveBus = function () {
    ...
    if ($this->bus_layout_id) {
        ...
        $totalSeats = BusLayoutSeat::where('bus_layout_id', $layoutId)
    ...
    @if ($showFormModal)
        <div x-data x-cloak class="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <div x-transition:enter="transition ease-out duration-300 transform"
                ...
                <div class="flex-1 overflow-y-auto min-h-0 p-6 space-y-8 pb-32 sm:pb-6">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-5">
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <span class="text-[11px] font-bold {{ $is_active ? 'text-emerald-600' : 'text-gray-400' }}">Aktif</span>
                                <div class="relative">
                                    <input type="checkbox" wire:model="is_active" class="sr-only peer">
                                    <div class="w-10 h-5 bg-gray-200 peer-checked:bg-emerald-500 rounded-full transition-colors"></div>
                                    <div class="absolute left-[2px] top-[2px] w-4 h-4 bg-white rounded-full shadow-sm transition-transform peer-checked:translate-x-5"></div>
                                </div>
                            </label>
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4 bg-gray-50/50 p-5 rounded-2xl border border-gray-100/50">
                            <div>
                                <label class="block text-[11px] font-bold text-gray-500 mb-1.5 uppercase tracking-wide">Nama Bus</label>
                            ...
                            <div>
                                <label class="block text-[11px] font-bold text-gray-500 mb-1.5 uppercase tracking-wide">Plat Nomor <span class="text-red-500">*</span></label>
                                ...
                                @error('plate_number')
                                    <p class="text-red-500 text-[10px] font-medium mt-1.5 flex items-center gap-1"><x-heroicon-s-exclamation-circle class="w-3 h-3"/> {{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                <label class="block text-[11px] font-bold text-gray-500 mb-1.5 uppercase tracking-wide">Merek</label>
                            ...
                            <div>
                                <label class="block text-[11px] font-bold text-gray-500 mb-1.5 uppercase tracking-wide">No. Mesin</label>
                                <input type="text" wire:model="machine_number" placeholder="Opsional"
                                    class="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-200 bg-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all placeholder-gray-300 uppercase">
                            </div>
                            <div>
                                <label class="block text-[11px] font-bold text-gray-500 mb-1.5 uppercase tracking-wide">No. Rangka</label>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-4 mt-2">
                            ...
                            <a href="{{ route('settings.bus-layouts') }}" class="text-[10px] font-bold text-purple-600 bg-purple-50 hover:bg-purple-100 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1" wire:navigate>
                        ...
                        <div class="bg-gray-50/50 p-5 rounded-2xl border border-gray-100/50">
                            ...
                            @if ($this->selectedLayoutPreview)
                                ...
                                <div class="mt-5 bg-white rounded-xl p-4 border border-purple-100 shadow-[0_2px_10px_-4px_rgba(147,51,234,0.1)]">
                                    <div class="flex items-center justify-between mb-4 border-b border-gray-50 pb-3">
                                        <div class="flex items-center gap-2">
                                            <div class="w-2 h-2 rounded-full bg-emerald-500"></div>
                                            <p class="text-xs font-bold text-gray-600 uppercase tracking-wide">Preview Denah</p>
                                        </div>
                                        <span class="text-[10px] font-black text-purple-700 bg-purple-100 px-2.5 py-1 rounded-md tracking-wide">
                                            {{ $preview->total_seats }} KURSI
                                        </span>
                                    </div>
                                    <div class="flex justify-center">
                                        <div class="bg-gray-50 p-3 rounded-2xl border border-gray-200/60 inline-block overflow-x-auto max-w-full">
                                            <div class="grid gap-2" style="grid-template-columns: repeat({{ $preview->total_columns }}, minmax(0, 1fr));">
                                                @foreach ($preview->seats as $seat)
                                                    <div class="w-10 h-10 rounded-xl flex items-center justify-center text-[11px] font-bold shadow-sm transition-transform hover:scale-105 cursor-default
                                    ...
                                    <div class="mt-4 flex flex-wrap justify-center gap-4 text-[10px] font-medium text-gray-400 border-t border-gray-50 pt-3">
                                        <div class="flex items-center gap-1.5"><div class="w-3 h-3 rounded bg-white border-2 border-emerald-500"></div> Kursi Penumpang</div>
                                        <div class="flex items-center gap-1.5"><div class="w-3 h-3 rounded bg-indigo-600"></div> Supir</div>
                                        <div class="flex items-center gap-1.5"><div class="w-3 h-3 rounded bg-amber-100 border-2 border-amber-400"></div> Pintu</div>
                                    </div>
                                </div>
                            @endif
                        </div>
                ...
                {{-- Footer (Sticky) --}}
                <div class="p-5 bg-white border-t border-gray-100 shrink-0 flex gap-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.02)]">
