    ...
    <x-layouts.app title="Detail Jadwal">
        <div class="px-4 pt-0 pb-24 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                {{-- Header Status --}}
                <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between" style="background: linear-gradient(135deg, #f8fafc, #f1f5f9);">
                    <div class="flex items-center gap-2">
                        <x-heroicon-s-map class="w-4 h-4 text-primary-600" />
                        <span class="text-[11px] font-bold text-gray-800 uppercase tracking-widest">Informasi Perjalanan</span>
                    </div>
                ...
                <div class="p-5 space-y-5">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="flex-1 min-w-0">
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Berangkat</p>
                            <p class="text-base font-bold text-gray-800 leading-tight">{{ $scheduleModel->route->originAgent->city ?? 'N/A' }}</p>
                            <p class="text-[11px] text-gray-500 mt-0.5 truncate">{{ $scheduleModel->route->originAgent->name ?? '-' }}</p>
                            <p class="text-lg font-black text-primary-600 mt-2 leading-none">{{ \Carbon\Carbon::parse($scheduleModel->departure_time)->format('H:i') }}</p>
                            <p class="text-[10px] font-semibold text-gray-400 mt-0.5">{{ $scheduleModel->departure_date->format('d M y') }}</p>
                        </div>
                        ...
                        <div class="flex flex-col items-center px-1">
                            ...
                            @if($scheduleModel->route->distance_km)
                                <span class="text-[10px] font-bold text-gray-400">{{ $scheduleModel->route->distance_km }} km</span>
                            @endif
                        </div>
                        
                        <div class="flex-1 min-w-0 text-right">
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Tiba (Est)</p>
                            <p class="text-base font-bold text-gray-800 leading-tight">{{ $scheduleModel->route->destinationAgent->city ?? 'N/A' }}</p>
                            <p class="text-[11px] text-gray-500 mt-0.5 truncate">{{ $scheduleModel->route->destinationAgent->name ?? '-' }}</p>
                            <p class="text-lg font-black text-emerald-600 mt-2 leading-none">{{ $scheduleModel->arrival_time ? \Carbon\Carbon::parse($scheduleModel->arrival_time)->format('H:i') : '-' }}</p>
                            <p class="text-[10px] font-semibold text-gray-400 mt-0.5">{{ $scheduleModel->arrival_date ? $scheduleModel->arrival_date->format('d M y') : '-' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 pt-4 border-t border-dashed border-gray-200">
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 uppercase mb-1 flex items-center gap-1"><x-heroicon-s-truck class="w-3 h-3"/> Armada</p>
                            <p class="text-sm font-bold text-gray-800 leading-tight">{{ $scheduleModel->bus->name ?? 'N/A' }} 
                            <br>
                            <span class="text-xs font-normal text-gray-500">({{ $scheduleModel->bus->plate_number ?? '-' }})</span></p>
                            <p class="text-[11px] text-gray-500 mt-1">Supir: <span class="font-semibold text-gray-800">{{ $scheduleModel->driver->name ?? 'Belum Ditentukan' }}</span></p>
                        </div>
                        <div class="text-right">
                            <p class="text-[10px] font-bold text-gray-400 uppercase mb-1 flex items-center justify-end gap-1">Biaya & Kursi <x-heroicon-s-ticket class="w-3 h-3"/></p>
                            <p class="text-sm font-black text-primary-700 leading-tight">Rp {{ number_format($scheduleModel->price, 0, ',', '.') }}<span class="text-[10px] font-normal text-gray-500">/kursi</span></p>
                            <p class="text-[11px] text-gray-500 mt-1">Sisa Kursi: <span class="font-bold {{ $scheduleModel->available_seats <= 2 ? 'text-red-500' : 'text-emerald-600' }}">{{ $scheduleModel->available_seats }}</span></p>
                        </div>
                    </div>
                </div>
            </div>
